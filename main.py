...
def process_file(file_path, output_dir, force_vision=False, max_concurrent=2, images_per_batch=1,
    ...
    try:
        ...
        if isinstance(processor, VisionDocumentProcessor) and doc_type == DocumentType.PDF:
            ...
            print(
                f"Processed PDF with {max_concurrent} concurrent workers, dynamic batching: {dynamic_batching}")
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert documents to structured markdown")

    parser.add_argument("input", nargs="?",
                        help="Input file or directory path")
    parser.add_argument(
        "--output", "-o", help="Output directory path", default="output")
